...
#load data
cv_file_path = r'/Users/austinjewett/Desktop/Purdue/AAE590/HW 3/DataSet_Disp_Import_csv.csv'
df_cv = pd.read_csv(cv_file_path)

#indep vars
X_cv = df_cv.iloc[:, 0:3].values

#dep vars + log transform
...
for train_index, test_index in kf.split(X_cv):
    ...
    #standardize data (both indep and dep for ANN)
    ...
    #build ANN w/ hyperparams set in HW 5 (no changes)
    ...
    #early stoping for optimum epoch number
...
plt.title('MAE & RMSE per Fold (disp) (ANN)')
plt.legend()
plt.grid(True)
