...
#load data
file_path = r"/Users/austinjewett/Desktop/Purdue/AAE590/HW 3/DataSet_Stress_Import_csv.csv"
...
for fold, (train_index, test_index) in enumerate(kf.split(X), 1):
    ...
    #std indep vars (dep already log transformed before)
    ...
    #built model (same params as defined in HW 3)
    with pm.Model() as model_cv:
        alpha_cv = pm.Normal('alpha', mu=6, sigma=3)
        betas_cv = pm.Uniform('betas', lower=-1, upper=2, shape=X_train_cv_scaled.shape[1])
        sigma_cv = pm.HalfNormal('sigma', sigma=1)
        ...
        trace_cv = pm.sample(2000, cores=1, tune=1000, return_inferencedata=True, progressbar=False)
...
plt.title('RMSE & MAE per Fold (stress) (BLR)')
plt.legend()
plt.grid(True)
...
plt.xlabel('Fold Number')
plt.ylabel('R² Score')
plt.title('R² per Fold (stress) (BLR)')
plt.legend()
plt.grid(True)
